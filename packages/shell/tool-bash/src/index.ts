...
function bashDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  return base + ' IMPORTANT: Do NOT set `sandbox_permissions` or `justification` unless the command '
    + 'result contains `[sandbox: file access denied under <mode> mode]`. These fields are ONLY for '
    + 'retrying a denied command — never use them proactively or speculatively. '
    + 'When a command is denied and a wider mode would let it succeed, escalate immediately in the '
    + 'same turn — the one sanctioned exception to a denial: retry the exact same command once with '
